...
public class TutorialMngInCompose : MonoBehaviour
{
    ...
    public IEnumerator LoadTextOneByOne(string inputTextString, Text inputTextUI, float eachTime = 0.1f, bool canClickSkip = true)
    {
        ...
        if (textOrder < 7 && textOrder!= 4 && textOrder!=6)
        {
        ...
        if (textOrder == 3)
        {
            flipButton.transform.SetParent(parentObject.transform);
        }

        if(textOrder == 5 || textOrder == 8 || textOrder == 7)
        {
